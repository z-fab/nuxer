...
class EnterrarAnimal:
    ...
    def __call__(self) -> UseCaseResponse:
        ...
        response_can_burial = user_animal_service._can_burial_animal_entity(user_id=user.id, user_animal=user_animal)

        if not response_can_burial.success:
            logger.error(f"[Enterrar Animal] Erro ao enterrar animal: {response_can_burial}")
            return UseCaseResponse(
                success=False, code=self.code, error_code=response_can_burial.error, data={"apelido": user.apelido}
            )
        ...
        if not response_transaction.success:
            logger.error(f"[Enterrar Animal] Erro ao enterrar animal: {transaction_service}")
            return UseCaseResponse(
                success=False,
                code=self.code,
        ...
        if not service_response.success:
            logger.error(f"[Enterrar Animal] Erro ao enterrar animal: {service_response}")
            return UseCaseResponse(
        ...
        logger.info(f"[Enterrar Animal] {service_response.data}")
