...
class AlterarSaldo:
    ...
    def __call__(self) -> UseCaseResponse:
        ...
        user = user_respository.get_user_by_slack_id(self.user_id)
        ...
        if response.success:
            logger.info(
                f"Alteração de Carteira realizada de {self.user_id} para {args['to_slack_id']}: {args['value']} F₵ - {args['description']}"
            )
        ...
        return UseCaseResponse(success=False, code=self.code, data={}, error_code=response.error)
